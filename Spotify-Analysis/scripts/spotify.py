...
st.set_page_config(
    ...
    page_icon="Spotify_Primary_Logo_RGB_Green.png",  # Ruta de la imagen del ícono
    layout="wide"
)
...
@st.cache_data
def cargar_datos():
    data = pd.read_csv("spotify_clean.csv",delimiter = ",")
    ...
    ultimo_dia = data['snapshot_date'].max()
    
    # Obtener Fecha de inicio (cambiar dias segun se requiera)
    fecha_inicio_semana = ultimo_dia - pd.Timedelta(days=30)

    # Filtrar los datos para el ultimo snapshot (datos de un solo día)
    #data = data[data['snapshot_date'] == ultimo_dia]

    # Filtrar los datos de un rango de fechas
    data = data[(data['snapshot_date'] >= fecha_inicio_semana) & (data['snapshot_date'] <= ultimo_dia)]

    # Ordenamos Canciones por 'country' y 'rank' en orden descendente
    #data = data.sort_values(['country', 'past_week_rank'], ascending=[True, True])
    return (data)
...
geojson_data = cargar_geojson("custom.geo.json")
...
with st.sidebar:
    # Logo Spotify
    st.image("Spotify_Full_Logo_RGB_Black.png", width=220, use_container_width=False)
